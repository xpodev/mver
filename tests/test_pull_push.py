"""Tests for resver pull and resver push commands."""
...
def test_pull_executes_command(
    ...
    mock_run = mocker.patch("resver.executor.subprocess.run", return_value=mocker.Mock(returncode=0))
    result = runner.invoke(app, ["pull"])
    assert result.exit_code == 0, result.output
    assert mock_run.call_count == 2  # two resources


def test_pull_no_resver_yml_fails(
    monorepo: Path,
    app_dir: Path,
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
...
def test_pull_command_failure_halts(
    ...
    _write_app_cfg(app_dir, "production", "1.0.0")
    monkeypatch.chdir(app_dir)
    mocker.patch("resver.executor.subprocess.run", return_value=mocker.Mock(returncode=1))
    result = runner.invoke(app, ["pull"])
    assert result.exit_code == 1
...
def test_pull_no_command_fails_before_executing(
    ...
    app_subdir.mkdir()
    app_resver_dir = app_subdir / ".resver"
    app_resver_dir.mkdir()
    with open(app_resver_dir / "app.yml", "w") as f:
        yml.dump({"group": "g1", "version": "1.0.0"}, f)
    monkeypatch.chdir(app_subdir)
    mock_run = mocker.patch("resver.executor.subprocess.run")
    result = runner.invoke(app, ["pull"])
    assert result.exit_code == 1
    mock_run.assert_not_called()


def test_pull_resource_pin_mode(
    monorepo: Path,
    app_dir: Path,
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
    ...
    """Pull works in resource-pin mode (direct resource versions, no group)."""
    ...
    result = runner.invoke(app, ["pull"])
    assert result.exit_code == 0, result.output
    assert mock_run.call_count == 2  # one per resource
