...
from resver.registry import find_registry, init_empty_registry, load_registry, save_registry
...
def test_find_registry_in_cwd(monorepo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    found = find_registry()
    assert found == monorepo / ".resver" / "registry.yml"
...
def test_load_registry_returns_data(monorepo: Path) -> None:
    data, path = load_registry(monorepo / ".resver" / "registry.yml")
    assert "resources" in data
    assert "groups" in data
    assert path == monorepo / ".resver" / "registry.yml"


def test_load_registry_empty_file(tmp_path: Path) -> None:
    resver_dir = tmp_path / ".resver"
    resver_dir.mkdir()
    reg = resver_dir / "registry.yml"
    reg.write_text("")
...
def test_save_registry_round_trip(monorepo: Path) -> None:
    data, path = load_registry(monorepo / ".resver" / "registry.yml")
    data["resources"]["new-model"] = {"description": "test", "versions": {}}
    save_registry(data, path)
    data2, _ = load_registry(path)
    assert "new-model" in data2["resources"]
